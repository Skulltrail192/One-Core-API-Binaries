...
/*Windows Code Replacement*/
ULONG NTAPI RtlIsCriticalSectionLocked(PRTL_CRITICAL_SECTION CriticalSection)
{
  return CriticalSection->LockCount & 1;
}

BOOL NTAPI RtlIsCriticalSectionLockedByThread(PRTL_CRITICAL_SECTION CriticalSection)
{
  BOOL result; // eax@1

  *(DWORD *)&result = CriticalSection->OwningThread == *(void **)(__readfsdword(24) + 36);
  return result;
}

PRTL_CRITICAL_SECTION NTAPI RtlEnableEarlyCriticalSectionEventCreation()
{
  PRTL_CRITICAL_SECTION result; // eax@1

  result = *(PRTL_CRITICAL_SECTION *)(__readfsdword(24) + 48);
  result[4].RecursionCount |= 0x10000000u;
  return result;
}
/*End Windows Code Replacement*/
/* EOF */
