...
TRACEHANDLE EtwpDumpHardwareConfig;
TRACEHANDLE EtwpDumpHWConfig;
int RtlpUnloadEventTrace = 0;
 ...
 * Stub to work on Windows.
 */
ULONG WINAPI EtwpSetHWConfigFunction(TRACEHANDLE SessionHandlem, BOOL enabled)
{
	TRACEHANDLE LocalSessionHandlem;
	if(enabled == TRUE)
	{
		LocalSessionHandlem = SessionHandlem;
		EtwpDumpHardwareConfig = SessionHandlem;
	}else{
...
int *NTAPI RtlGetUnloadEventTrace()
{
