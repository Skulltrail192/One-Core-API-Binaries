...
{
  const LARGE_INTEGER *timeout; // eax@1
  NTSTATUS status; // esi@1
  BOOL result; // eax@1
  LARGE_INTEGER number; // [sp+4h] [bp-8h]@1

  timeout = BaseFormatTimeOut(&number, dwMilliseconds);
  status = RtlSleepConditionVariableCS(ConditionVariable, CriticalSection, timeout);
  DbgPrint("Function: SleepConditionVariableCS. From ntdll function: RtlSleepConditionVariableCS. Status: %08x\n",status);
 ...
 * @implemented - need test
 */
HANDLE WINAPI CreateSemaphoreExW(LPSECURITY_ATTRIBUTES lpSemaphoreAttributes, LONG InitialCount, LONG MaximumCount, LPCWSTR lpName, DWORD dwFlags, DWORD DesiredAccess)
{
 ...
 * @implemented - need test
 */
HANDLE WINAPI CreateSemaphoreExA(LPSECURITY_ATTRIBUTES lpSemaphoreAttributes, LONG lInitialCount, LONG lMaximumCount, LPCTSTR lpName, DWORD dwFlags, DWORD dwDesiredAccess)
{
 ...
 * @implemented - need test
 */
HANDLE WINAPI CreateMutexExW(LPSECURITY_ATTRIBUTES lpMutexAttributes, LPCWSTR lpName, DWORD dwFlags, DWORD DesiredAccess)
{
  NTSTATUS status; // eax@4
 ...
 * @implemented - need test
 */
HANDLE WINAPI CreateMutexExA(LPSECURITY_ATTRIBUTES lpMutexAttributes, LPCTSTR lpName, DWORD dwFlags, DWORD DesiredAccess)
{
  BOOL result; // eax@2
