...

{
	...
	_SEH2_END;*/
	typedef void (*BASETHREADSTART)(LPTHREAD_START_ROUTINE lpStartAddress,
    LPVOID lpParameter);
	HINSTANCE hDLL = NULL;
	BASETHREADSTART basethreadstart;
	hDLL = LoadLibrary(L"KERNELFULL.DLL");
	basethreadstart = (BASETHREADSTART)GetProcAddress((HMODULE)hDLL, "BaseThreadStartThunk");
}

VOID
WINAPI
BasepProcessStartTrunk(
...

{
	...
	_SEH2_END;*/
	/*typedef void (*BASEPROCESSSTART)(LPTHREAD_START_ROUTINE lpStartAddress,
    LPVOID lpParameter);
	HINSTANCE hDLL = NULL;
	BASEPROCESSSTART baseprocessstart;
	hDLL = LoadLibrary(L"KERNELFULL.DLL");
	baseprocessstart = (BASEPROCESSSTART)GetProcAddress((HMODULE)hDLL, "BaseProcessStartThunk");
	baseprocessstart(lpStartAddress, lpParameter);*/
	//BaseProcessStartThunk((PPROCESS_START_ROUTINE)&lpStartAddress, &lpParameter);
}