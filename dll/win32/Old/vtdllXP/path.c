    ...
    _Out_ RTL_PATH_TYPE *PathType)
{
    ...
    *PathType = RtlPathTypeUnknown;
    ...
    TRACE("DOS length for filename: %lx %wZ\n", DosLength, FileName);
    if (DosLength != 0)
    {
        ...
        case RtlPathTypeDriveRelative:      /* c:foo   */
                ...
                CurDirName->Buffer[1] != L':')
            {
                ...
                switch (Status)
                {
                    ...
                    default:
                        TRACE("RtlQueryEnvironmentVariable_U returned 0x%08lx\n", Status);
            ...
            /* Fall through */
            TRACE("RtlPathTypeDriveRelative - Using fall-through to RtlPathTypeRelative\n");
                                         ...
                                         OUT PRTL_RELATIVE_NAME_U RelativeName)
{
    ...
    TRACE("Relative: %lx DosName: %wZ NtName: %p, PartName: %p, RelativeName: %p\n",
    ...
    {
        /* Invalid name, fail */
        TRACE("Invalid name: %lx Path Length: %lx\n", NameInvalid, PathLength);
...
NTSTATUS NTAPI RtlpDosPathNameToRelativeNtPathName_U( 	
