                                             ...
                                             const LARGE_INTEGER *timeout )
{
	...
	DbgPrint("Functions: RtlSleepConditionVariableCS. From NtWaitForKeyedEvent. Status: %08x\n",status );
...
void WINAPI RtlInitializeConditionVariable( RTL_CONDITION_VARIABLE *variable )
{
	 ...
	 if(variable==NULL)
		variable->Ptr = NULL;
	 status = NtCreateKeyedEvent(&keyed_event, -1, NULL, 0);
	 DbgPrint("Function: RtlInitializeConditionVariable. ConditionVariable initialized. Globalkeyed_event initialized. Status from NtCreateKeyedEvent: %08x\n", status);
}
...
void WINAPI RtlWakeAllConditionVariable( RTL_CONDITION_VARIABLE *variable )
{
	...
	{
        status = NtReleaseKeyedEvent( keyed_event, &variable->Ptr, FALSE, NULL );
		DbgPrint("Functions: RtlWakeAllConditionVariable. From NtReleaseKeyedEvent. Status: %08x\n",status );
	}
}
