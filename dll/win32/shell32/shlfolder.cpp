...
HRESULT 
WINAPI 
SHBindToObject(IShellFolder *psf, LPCITEMIDLIST pidl, IBindCtx *pbc, REFIID riid, void **ppv)
{
  boolean verification; // zf@4
  IShellFolder *object = NULL; // ecx@4
  HRESULT ret; // eax@5 MAPDST
  IShellFolder *ppshf; // [sp+14h] [bp-8h]@2

  *ppv = 0;
  if ( psf )
    ret = psf->QueryInterface(riid, (void **)&ppshf);
  else
    ret = SHGetDesktopFolder(&ppshf);
  if ( ret >= 0 )
  {
    verification = _ILIsEmpty(pidl) == 0;
    object = ppshf;
    if ( verification )
      ret = object->BindToObject((LPCITEMIDLIST)pidl, pbc, riid, ppv);
    else
      ret = object->QueryInterface(riid, ppv);
    if ( ret >= 0 && !*ppv )
      ret = E_FAIL;
    ppshf->Release();
  }
