...
#XP function only or for XP code
...
@ stdcall I_QueryTagInformation(long long long) advapinew.I_QueryTagInformation ;native on Server 2003, but, missing on XP
;@ stdcall RegGetValueA(long str str long ptr ptr ptr) ;native on Server 2003, but missing on XP (needed for code synchronization)
;@ stdcall RegGetValueW(long wstr wstr long ptr ptr ptr) ;native on Server 2003, but missing on XP (needed for code synchronization)
