 ...
 */
int WINAPI GetUserDefaultLocaleName(
 ...
 * @implemented - need test
 */
int WINAPI GetLocaleInfoEx(_In_opt_   LPCWSTR lpLocaleName,
					_In_      LCTYPE LCType,
					...
					_In_      int cchData)
{
	...
	for(i=0;i<3;i++)
	{
		if(lstrcmpW(lpLocaleName,LocaleList[i].code) == 0)
		{
...
)
{
	...
	for(i=0;i<2;i++)
	{
		if(lstrcmpiW(lpLocaleName,LocaleList[i].code) == 0)
		{
			count = 1;
		}
	}	
...
BOOL WINAPI IsNormalizedString(NORM_FORM NormForm, LPCWSTR lpString, int cwLength)
{
  ...
  ULONG error; // eax@2
  BOOL result; // eax@4
  BOOLEAN verify; // [sp+1h] [bp-1h]@1

  status = RtlIsNormalizedString(NormForm, lpString, cwLength, &verify);
  if ( status >= 0 )
  {
    result = verify == 1;
  }
  else
  {
    ...
    if ( *(DWORD *)(__readfsdword(24) + 52) == 2 )
...
{
  NTSTATUS status; // eax@1
  NTSTATUS otherStatus; // esi@1
  int result; // eax@2
  ULONG error; // eax@5

  status = RtlNormalizeString(NormForm, lpSrcString, cwSrcLength, lpDstString, &cwDstLength);
  otherStatus = status;
  if ( status < 0 )
  {
    error = RtlNtStatusToDosError(status);
    ...
    if ( *(DWORD *)(__readfsdword(24) + 52) == 2 )
      RtlSetLastWin32Error(87);
    if ( otherStatus == 0xC0000023 || otherStatus == 0xC0000717 )
      result = -cwDstLength;
    else
      result = 0;
  }
