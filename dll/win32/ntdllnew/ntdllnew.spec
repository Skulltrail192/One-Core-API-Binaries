...
@ stdcall RtlLCIDToCultureName(long wstr) ;this functions already have implementation on vtdll
...
@ stdcall RtlSleepConditionVariableCS(ptr ptr ptr) ;commented for now
@ stdcall RtlSleepConditionVariableSRW(ptr ptr ptr long) ;commented for now
...
@ stdcall RtlWow64EnableFsRedirectionEx(long ptr) ;commented for force use of ntdlllnew
@ stdcall RtlWakeConditionVariable(ptr) ;commented for now
...
;needed for vernelxp (Kernel32 Xp version)
@ stdcall LdrOpenImageFileOptionsKey(ptr long ptr)
...
@ stdcall LdrAlternateResourcesEnabled() ;unimeplemted.c
