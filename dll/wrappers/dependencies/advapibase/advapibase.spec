...
;@ stdcall AddMandatoryAce(ptr long long long ptr) ;don't need for now
...
;@ stdcall ControlServiceEx(ptr long long ptr) ;don't need for now 
...
;@ stdcall ElfDeregisterEventSourceW(ptr) ElfDeregisterEventSource  ;don't need for now
...
;@ stdcall GetSiteSidFromToken(ptr)  ;don't need for now
...
;@ stdcall LsaRegisterPolicyChangeNotification(long long) ;don't need for now
...
;@ stdcall LsaUnregisterPolicyChangeNotification(long long) ;don't need for now
...
;@ stdcall QuerySecurityAccessMask(long ptr)  ;don't need for now
...
;@ stdcall RegCopyTreeA(ptr str ptr) ;don't need for now
;@ stdcall RegCopyTreeW(ptr wstr ptr) ;don't need for now
...
;@ stdcall RegDeleteKeyValueA(ptr str str) ;don't need for now 
;@ stdcall RegDeleteKeyValueW(ptr wstr wstr) ;don't need for now
;@ stdcall RegDeleteTreeA(long str) ;don't need for now 
;@ stdcall RegDeleteTreeW(long wstr) ;don't need for now
...
;@ stdcall RegLoadMUIStringA(long str str long ptr long str) ;don't need for now
;@ stdcall RegLoadMUIStringW(long wstr wstr long ptr long wstr) ;don't need for now
...
;@ stdcall RegSetKeyValueA(long str str long ptr long) ;don't need for now
;@ stdcall RegSetKeyValueW(long wstr wstr long ptr long) ;don't need for now
...
;@ stdcall SetSecurityAccessMask(long ptr) ;don't need for now
...
@ stdcall SynchronizeWindows31FilesAndWindowsNTRegistry(long long long long) ;don't need for now
