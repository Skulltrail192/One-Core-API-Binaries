...
// HACK
    ...
    if ( !Ret ||
            !pRgn_Attr_Dest ||
            !pRgn_Attr_Src1 ||
            pRgn_Attr_Src1->AttrFlags > SIMPLEREGION )
        return NtGdiCombineRgn(hDest, hSrc1, hSrc2, CombineMode);

    /* Handle COPY and use only src1. */
    if ( CombineMode == RGN_COPY )
    {
        switch (pRgn_Attr_Src1->AttrFlags)
        {
        case NULLREGION:
    ...
    if ( !Ret ||
            !pRgn_Attr_Src2 ||
            pRgn_Attr_Src2->AttrFlags > SIMPLEREGION )
        return NtGdiCombineRgn(hDest, hSrc1, hSrc2, CombineMode);
    ...
    if ( CombineMode != RGN_AND)
    {
        ...
        if ( CombineMode > RGN_XOR) /* Handle DIFF. */
        {
            ...
            /* Now handle DIFF. */
            if ( pRgn_Attr_Src1->AttrFlags == NULLREGION )
            {
            ...
            if ( pRgn_Attr_Src2->AttrFlags != NULLREGION )
            {
    ...
    /* Handle AND.  */
    if ( pRgn_Attr_Src1->AttrFlags != NULLREGION &&
            pRgn_Attr_Src2->AttrFlags != NULLREGION )
    {
        Complexity = ComplexityFromRects( &pRgn_Attr_Src1->Rect, &pRgn_Attr_Src2->Rect);
    ...
    if (iMode == RGN_COPY)
    {
                ...
                pDc_Attr )
        {
            ...
            if ( pTeb->Win32ThreadInfo != NULL &&
                    ...
                    !(pEntry->AttrFlags & GDI_ENTRY_VALIDATE_VIS) )
            {
                if (!hrgn ||
                        (hrgn && pRgn_Attr && pRgn_Attr->AttrFlags <= SIMPLEREGION) )
                {
          ...
          LPRECT prcOut)
{
    ...
    if (Rgn_Attr->AttrFlags == NULLREGION)
    {
        prcOut->left   = 0;
        prcOut->top    = 0;
        prcOut->right  = 0;
...
//  if (!GdiGetHandleUserData((HGDIOBJ) hrgn, GDI_OBJECT_TYPE_REGION, (PVOID) &pRgn_Attr))
    ...
    if ( pRgn_Attr->AttrFlags == NULLREGION)
        return pRgn_Attr->AttrFlags;

    if ( pRgn_Attr->AttrFlags != SIMPLEREGION)
        return NtGdiOffsetRgn(hrgn,nXOffset,nYOffset);
           ...
           int nBottomRect)
{
    ...
    Rgn_Attr->AttrFlags |= ATTR_RGN_DIRTY ;
    Rgn_Attr->AttrFlags = SIMPLEREGION;
    return TRUE;
}
