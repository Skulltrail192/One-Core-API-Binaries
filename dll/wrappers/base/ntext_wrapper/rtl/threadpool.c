...
struct threadpool_instance
{
    ...
    struct
    {
        PRTL_CRITICAL_SECTION    critical_section;
        HANDLE              mutex;
        HANDLE              semaphore;
 ...
 */
static void CALLBACK timerqueue_thread_proc(PVOID param )
{
...
static NTSTATUS tp_group_alloc( struct threadpool_group **out )
{
    ...
    group->cs.DebugInfo->SpareWORD = (DWORD_PTR)(__FILE__ ": threadpool_group.cs");
...
static BOOL tp_group_release( struct threadpool_group *group )
{
    ...
    group->cs.DebugInfo->SpareWORD = 0;
 ...
 */
VOID WINAPI TpCallbackLeaveCriticalSectionOnCompletion( TP_CALLBACK_INSTANCE *instance, PRTL_CRITICAL_SECTION crit )
{
...
BOOL WINAPI TpSetPoolMinThreads( TP_POOL *pool, DWORD minimum )
{
    ...
    while (this->num_workers < minimum)
    {
        HANDLE thread;
        status = RtlCreateUserThread( NtCurrentProcess(), NULL, FALSE, 0, 0, 0,
                                      (PTHREAD_START_ROUTINE)threadpool_worker_proc, this, &thread, NULL );
        if (status != STATUS_SUCCESS)
            break;
