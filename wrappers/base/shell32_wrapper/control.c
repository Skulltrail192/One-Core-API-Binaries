...
#ifdef __REACTOS__
    StringCchCopy((STRSAFE_LPSTR)fileBuffer, MAX_PATH, (STRSAFE_LPSTR)applet->cmd);
    if (PathIsFileSpecW(fileBuffer))
        SearchPath(NULL, (LPCSTR)fileBuffer, NULL, MAX_PATH, (LPCSTR)fileBuffer, NULL);
    ActCtx.lpSource = fileBuffer;
    ActCtx.lpResourceName = (LPCWSTR)123;
    applet->hActCtx = CreateActCtx((PCACTCTXA)&ActCtx);
