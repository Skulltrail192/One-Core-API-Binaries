...
BOOL WINAPI CheckTokenMembership( HANDLE token, PSID sid_to_check, PBOOL is_member )
{
    ...
    *is_member = FALSE;
    ...
    token_groups = RtlAllocateHeap(RtlGetProcessHeap(),
								   0,
								   size);
    if (!token_groups)
    {
...
exit:
    RtlFreeHeap(GetProcessHeap(),0,token_groups);
                                   ...
                                   DWORD restrict_sid_count, SID_AND_ATTRIBUTES *restrict_sids, HANDLE *ret )
{
    ...
    {
        if (!(nt_disable_sids =  RtlAllocateHeap(RtlGetProcessHeap(),0, offsetof( TOKEN_GROUPS, Groups[disable_sid_count] ) ))) goto out;
    ...
    {
        if (!(nt_privs = RtlAllocateHeap(RtlGetProcessHeap(),0, offsetof( TOKEN_GROUPS, Groups[delete_priv_count] ) ))) goto out;
    ...
    {
        if (!(nt_restrict_sids = RtlAllocateHeap(RtlGetProcessHeap(),0, offsetof( TOKEN_GROUPS, Groups[restrict_sid_count] ) ))) goto out;
...
out:
	RtlFreeHeap(GetProcessHeap(),0,nt_disable_sids);
	RtlFreeHeap(GetProcessHeap(),0,nt_privs);
	RtlFreeHeap(GetProcessHeap(),0,nt_restrict_sids);
...
BOOL WINAPI IsTokenRestricted( HANDLE token )
{
    ...
    groups = RtlAllocateHeap(RtlGetProcessHeap(),0, size);
    if (!groups)
    {
                                           ...
                                           ULONG flags, HANDLE token, PGENERIC_MAPPING mapping )
{
    ...
    if (!(buffer = RtlAllocateHeap(RtlGetProcessHeap(),0, needed ))) return FALSE;
...
{
    return RtlFirstFreeAce( acl, (PACE *)ace );
}
