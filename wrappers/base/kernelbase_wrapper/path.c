...
Abstract:
    ...
    This module implements path functions of shlwapi in Win32 API Base - Copied from wine's kernelbase path.c

Author:

    Skulltrail 01-July-2021
...
BOOL WINAPI PathMatchSpecA(const char *path, const char *mask)
{
    ...
    RtlFreeHeap(GetProcessHeap(), 0,  maskW );
    return ret;
}
...
static DWORD get_scheme_code(const WCHAR *scheme, DWORD scheme_len)
{
    ...
    for (i = 0; i < ARRAY_SIZE(url_schemes); ++i)
    {
        if (scheme_len == lstrlenW(url_schemes[i].scheme_name)
                && !_wcsnicmp(scheme, url_schemes[i].scheme_name, scheme_len))
...
HRESULT WINAPI PathCreateFromUrlW(const WCHAR *url, WCHAR *path, DWORD *pcchPath, DWORD dwReserved)
{
    ...
    if (*pcchPath < len)
        tpath = RtlAllocateHeap(GetProcessHeap(), 0, len * sizeof(WCHAR));
    else
        tpath = path;
    ...
    case 3:
        ...
        break;
    case 2:
        if (lstrlenW(src) >= 10 && !_wcsnicmp( src, L"localhost", 9) && (src[9] == '/' || src[9] == '\\'))
        {
    ...
    if (tpath != path)
        RtlFreeHeap(GetProcessHeap(), 0, tpath);
...
HRESULT WINAPI UrlEscapeA(const char *url, char *escaped, DWORD *escaped_len, DWORD flags)
{
    ...
    {
        escapedW = RtlAllocateHeap(GetProcessHeap(), 0, lenW * sizeof(WCHAR));
    ...
    if (escapedW != bufW)
        RtlFreeHeap(GetProcessHeap(), 0, escapedW);
    RtlFreeUnicodeString(&urlW);
    return hr;
}
...
HRESULT WINAPI UrlEscapeW(const WCHAR *url, WCHAR *escaped, DWORD *escaped_len, DWORD flags)
{
    ...
    dst_ptr = dst = RtlAllocateHeap(GetProcessHeap(), 0, *escaped_len * sizeof(WCHAR));
    if (!dst_ptr)
        return E_OUTOFMEMORY;
    ...
    for (src = url; *src; )
    {
        ...
        if ((int_flags & WINE_URL_COLLAPSE_SLASHES) && src == url + parsed_url.cchProtocol + 1)
        {
            ...
            if (slashes == 2 && !_wcsnicmp(src, L"localhost", 9)) { /* file://localhost/ -> file:/// */
    ...
    *escaped_len = needed;
    ...
    RtlFreeHeap(GetProcessHeap(), 0, dst_ptr);
    return hr;
}
...
HRESULT WINAPI UrlCanonicalizeA(const char *src_url, char *canonicalized, DWORD *canonicalized_len, DWORD flags)
{
    ...
    canonical = RtlAllocateHeap(GetProcessHeap(), 0, *canonicalized_len * sizeof(WCHAR));
    if (!url || !canonical)
    {
        RtlFreeHeap(GetProcessHeap(), 0, url);
        RtlFreeHeap(GetProcessHeap(), 0, canonical);
        return E_OUTOFMEMORY;
    }
...
HRESULT WINAPI UrlCanonicalizeW(const WCHAR *src_url, WCHAR *canonicalized, DWORD *canonicalized_len, DWORD flags)
{
    ...
    url_copy = RtlAllocateHeap(GetProcessHeap(), 0, nByteLen + sizeof(L"file:///"));
    if (!url_copy)
    {
        RtlFreeHeap(GetProcessHeap(), 0, url);
        return E_OUTOFMEMORY;
    ...
    while (*wk1)
    {
        ...
        default:
            ...
            RtlFreeHeap(GetProcessHeap(), 0, url_copy);
            RtlFreeHeap(GetProcessHeap(), 0, url);
            return E_INVALIDARG;
        }
        *wk2 = '\0';
...
HRESULT WINAPI UrlApplySchemeA(const char *url, char *out, DWORD *out_len, DWORD flags)
{
    ...
    inW = RtlAllocateHeap(GetProcessHeap(), 0, 2 * INTERNET_MAX_URL_LENGTH * sizeof(WCHAR));
    ...
    if (hr != S_OK)
    {
        RtlFreeHeap(GetProcessHeap(), 0, inW);
        return hr;
    }
...
static HRESULT url_create_from_path(const WCHAR *path, WCHAR *url, DWORD *url_len)
{
    ...
    new_url = RtlAllocateHeap(GetProcessHeap(), 0, (lstrlenW(path) + 9) * sizeof(WCHAR)); /* "file:///" + path length + 1 */
    ...
    RtlFreeHeap(GetProcessHeap(), 0, new_url);
    return hr;
}
...
HRESULT WINAPI UrlCreateFromPathA(const char *path, char *url, DWORD *url_len, DWORD reserved)
{
    ...
    {
        urlW = RtlAllocateHeap(GetProcessHeap(), 0, lenW * sizeof(WCHAR));
    ...
    if (urlW != bufW)
        RtlFreeHeap(GetProcessHeap(), 0, urlW);
...
HRESULT WINAPI UrlCombineA(const char *base, const char *relative, char *combined, DWORD *combined_len, DWORD flags)
{
    ...
    baseW = RtlAllocateHeap(GetProcessHeap(), 0, 3 * INTERNET_MAX_URL_LENGTH * sizeof(WCHAR));
    ...
    {
        *combined_len = len;
        RtlFreeHeap(GetProcessHeap(), 0, baseW);
        return hr;
    }
...
HRESULT WINAPI UrlCombineW(const WCHAR *baseW, const WCHAR *relativeW, WCHAR *combined, DWORD *combined_len, DWORD flags)
{
    ...
    preliminary = RtlAllocateHeap(GetProcessHeap(), 0, 3 * INTERNET_MAX_URL_LENGTH * sizeof(WCHAR));
    ...
    *preliminary = '\0';
    ...
    else do
    {
        ...
        if ((work = wcspbrk(base.pszSuffix + sizeloc, L"#?")))
        {
            ...
            {
                if (!_wcsnicmp(work - 4, L".htm", 4)) manual_search = TRUE;
            }

            if (!manual_search && work - base.pszSuffix > 5)
            {
                if (!_wcsnicmp(work - 5, L".html", 5)) manual_search = TRUE;
            }
        }
    ...
    RtlFreeHeap(GetProcessHeap(), 0, preliminary);
    return hr;
}
