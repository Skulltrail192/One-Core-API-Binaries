...
CONST UCHAR BitsSetTotal[] = { 0,1,1,2,1,2,2,3 };
    ...
    )
{
    ...
    setBits  = BitsSetTotal[ (UCHAR)Target ];
...
*/
{
    PSCSI_REQUEST_BLOCK tempTail, foundSrb;
    ULONG srbsFound;
    ...
    if (Queue->Tail == NULL) {
        ...
        tempTail = (PSCSI_REQUEST_BLOCK) Queue->Tail;        
        if (tempTail->NextSrb == NULL) {    //Verify SRBs are not about to be severed from the Queue
            tempTail->NextSrb = Srb;
        } else {
            NT_ASSERT(FALSE); ChannelExtension = NULL; ChannelExtension->PortNumber++;
        }
        Queue->Tail = (PVOID) Srb;            
        if (Srb->NextSrb != NULL) {         //Verify NextSrb is not in use by anyone
            NT_ASSERT(FALSE); ChannelExtension = NULL; ChannelExtension->PortNumber++;
        }
    }
    Queue->CurrentDepth++;
    ...
    if (Queue->CurrentDepth != srbsFound ) {
        //intentional bugcheck to check queuing errors
        NT_ASSERT(FALSE); ChannelExtension = NULL; ChannelExtension->PortNumber++;
    }
...
{
    PSCSI_REQUEST_BLOCK nextSrb, foundSrb;
    ULONG srbsFound;
    ...
    nextSrb = (PSCSI_REQUEST_BLOCK) Queue->Head;
    Queue->Head = (PVOID) nextSrb->NextSrb;
    nextSrb->NextSrb = NULL;                //This should have been NULL to start with, return it to it's NULL state
    ...
    if (Queue->Head == NULL) {
        ...
        foundSrb = (PSCSI_REQUEST_BLOCK) Queue->Head;
        srbsFound = 1;
        foundSrb = foundSrb->NextSrb;
        while (foundSrb) {
            srbsFound++;
    ...
    for (i = 0; i <= (adapterExtension->CAP.NCS); i++) {
        if( ( ChannelExtension->SlotManager.CommandsToComplete & (1 << i) ) > 0) {
            ...
            if (slotContent->Srb == NULL) {
                ...
                //The completed slot has no IRB so it can not be completed back to ATAport.
          ...
          //2.2 Set the status
            if( (SrbStatus == SRB_STATUS_SUCCESS) &&
                ...
                (!IsNCQCommand(slotContent->Srb)) && 
                (slotContent->CmdHeader->PRDBC != *SrbGetDataTransferLength(slotContent->Srb)) ) {
                //
                if (slotContent->CmdHeader->PRDBC < *SrbGetDataTransferLength(slotContent->Srb)) {
                    //buffer underrun,
                    *SrbGetDataTransferLength(slotContent->Srb) = slotContent->CmdHeader->PRDBC;
...
--*/
{
  ...
  //1.2 Specail case mapping of NCQ
    if( IsNCQCommand(SlotContent->Srb) ){
...
--*/
{
  ...
  //1.1 Memcopy CDB into ACMD
    ...
    dataLength = *SrbGetDataTransferLength(SlotContent->Srb);
  ...
  //2.2 Populate DMA bit properly, only use it for data transfer
    if (IsDmaCommand(srbExtension->Flags)) {
        ...
        if ( ((ChannelExtension->DeviceExtension[0].IdentifyPacketData->ReservedWords54[8] & (1 << 15)) != 0) &&
...
--*/
{
    ...
    for (i = 0; i < sgl->NumberOfElements; i++) { 
      ...
      //1.3 Verify that the DataLength is even
        ...
        if( (sgl->List[i].Length & 1) == 0 ) {                    //therefore length must be even here
          //2.3 Set Datalength in the PRDT entries
        ...
        } else if (sgl->List[i].Length <= *SrbGetDataTransferLength(SlotContent->Srb)) {
            ...
            *SrbGetDataTransferLength(SlotContent->Srb) = *SrbGetDataTransferLength(SlotContent->Srb) - 1;
...
--*/
{
    ...
    PSCSI_REQUEST_BLOCK srb = SlotContent->Srb;
  ...
  //3 Find an available slot/tag (AHCI 1.1 Section 5.5.1)
    ...
    if ( srbExtension->QueueTag > ChannelExtension->AdapterExtension->CAP.NCS ) {
        ...
        if (!IsMiniportInternalSrb(ChannelExtension, Srb)) {
            ...
            StorPortDeviceBusy(ChannelExtension->AdapterExtension, Srb->PathId, Srb->TargetId, Srb->Lun, min(8, ChannelExtension->AdapterExtension->CAP.NCS));
        }
...
#ifdef DBG
    {
        ...
        if( ( ( (1 << srbExtension->QueueTag) & ci) > 0) || ( ( (1 << srbExtension->QueueTag) & sact) > 0) ) {
            ...
            if (!IsMiniportInternalSrb(ChannelExtension, Srb)) {
                ...
                StorPortPauseDevice(ChannelExtension->AdapterExtension, Srb->PathId, Srb->TargetId, Srb->Lun, 1);   //pause for 1 seconds.
            }
            Srb->SrbStatus = SRB_STATUS_BUSY;
            MarkSrbToBeCompleted(Srb);
            AhciCompleteRequest(ChannelExtension, Srb, AtDIRQL);
...
*/
{
  ...
  //1.1 Update FUA tracking flag if needed
    if( !IsReturnResults(srbExtension->Flags) &&
        ...
        (((PCDB)Srb->Cdb)->CDB10.ForceUnitAccess == 1) &&
    ...
    if (LogCommand(ChannelExtension->AdapterExtension->LogFlags)) {
        ...
        cmdHistory->Function = slotContent->Srb->Function;
    ...
    )
{
    ...
    if ( (srbExtension == NULL) || (srbExtension->AtaFunction != 0) ||
         (FailingSrb->SenseInfoBuffer == NULL) || (FailingSrb->SenseInfoBufferLength == 0) ) {
    ...
    senseSrb->DataBuffer = FailingSrb->SenseInfoBuffer;
    senseSrb->DataTransferLength = FailingSrb->SenseInfoBufferLength;
    ...
    dataBufferPhysicalAddress = StorPortGetPhysicalAddress(ChannelExtension->AdapterExtension, NULL, FailingSrb->SenseInfoBuffer, &length);
    ...
    srbExtension->LocalSgl.List[0].Length = srbExtension->DataTransferLength; 
    ...
    do {
        ...
        if (srb != NULL) {
            ...
            if (completionRoutine != NULL) {
                ...
                NT_ASSERT(FALSE); //Srb without Completion Routine should not be put in the CompletionQueue.
            }

            if (completeSrb && !IsMiniportInternalSrb(channelExtension, srb) ) {
